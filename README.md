...
|---|---|
| **C** | `typedef struct` with `m_` fields, error codes, function-pointer visitor/predicate pattern (the C closure idiom), `static` helpers, `NULL` handling |
...
| **Objective-C** | `@interface`/`@implementation`, block typedefs, `copy` block property, inline predicate block, `__weak`/`__strong` retain-cycle pattern, dot notation vs message syntax |
